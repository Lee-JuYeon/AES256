package com.cavss.aes256.cipher
...
object AESHelper {
    ...
    private const val logTag : String = "AESHelper"
    ...
    fun keystoreEncrypt(dataToEncrypt : ByteArray) : HashMap<String, ByteArray>{
        ...
        try {
            ...
            Log.d(logTag, "keystore 암호화 하는 도중 : ${e.message}")
        }finally {
            return map
        }
    }
    ...
    fun keystoreDecrypt(map: HashMap<String, ByteArray>) : ByteArray?{
        ...
        try {
            ...
            Log.d(logTag,"$logTag, keystore decrypt 중 Exception : ${e.message}")
        } catch (e: Throwable) {
            Log.d(logTag,"$logTag, keystore decrypt 중 Throwable : ${e.message}")
        }finally {
            return decrypted
        }
    }
    ...
    fun keystoreTest(testMessage : String, afterFunc : (decrypted : String) -> Unit) {
        try {
            ...
            Log.d(logTag, "keystore 테스트 하는 도중 : ${e.message}")
        }catch (e: InvalidKeySpecException) {
            Log.d(logTag, "keystore 테스트 하는 도중 키를 못가져옴 : ${e.message}")
        }
    }
    fun keystoreSetting(){
        try {
